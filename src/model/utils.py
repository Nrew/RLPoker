            ...
            if uuid: # Ensure uuid exists
                 ...
                 if player_pos != -1:
                     if "SMALLBLIND" in action_type or (action_type == "POST" and amount == small_blind_amount):
                         ...
                         small_blind_amount = max(small_blind_amount, amount) 
...
def extract_state(hole_card: Optional[List[str]],
    ...
    try:
        ...
        # Calculate total pot size including side pots
        for side_pot in pot_info.get('side', []): 
        ...
        bb: float = max(big_blind_amount, config.EPSILON) 
        ...
        if street_idx > 0 and hole_card and len(hole_card) == 2:
            try:
                ...
                if hole_cards_int and community_cards_int is not None:
                    num_opponents_for_est: int = max(1, num_active_players - 1)
                    win_rate = estimate_hole_card_win_rate(
                        nb_simulation=config.NB_ESTIMATIONS, # Lower simulation count for speed
        ...
        # Normalize SPR: clipping and scaling to [0, 1]
        ...
        if valid_actions:
            ...
            # Estimate from history if valid_actions not provided
            action_histories: Dict[str, List[Dict[str, Any]]] = round_state.get('action_histories', {})
                ...
                for action in current_street_history:
                    ...
                    if uuid and uuid in current_bets:
                        if action_type in ['bet', 'raise']:
                            current_bets[uuid] = amount # Bet/Raise is the total bet
                        elif action_type == 'call':
                            current_bets[uuid] = amount # Call is the amount to call
                        elif action_type in ['smallblind', 'bigblind', 'post']:
                            current_bets[uuid] = amount # Post is the amount posted
        ...
        state_list.append(np.clip(norm_call_amount_pot, 0.0, 2.0))
        ...
        # Normalize by capping at a reasonable number (e.g., 5) to avoid extreme values
...
def map_action_to_poker(action_idx: int,
    ...
    # Check if the (proposed_action_name, proposed_amount_int) pair is actually valid
    chosen_action_is_valid: bool = False
    final_action_name: str = proposed_action_name
    
    # Ensure non-negative int
    final_amount_int: int = int(max(0, proposed_amount_int))
    ...
    if not chosen_action_is_valid:
        ...
        if can_call and call_action_info: # If can call, default to call
            ...
            final_amount_int = 0
             ...
             final_amount_int = raise_action_info['amount']['min']
